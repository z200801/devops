...
curl http://localhost:5000/v2/swarm-nginx/tags/list

# Get manifest (image metadata)
curl http://localhost:5000/v2/swarm-nginx/manifests/v1
```
...
DIGEST=$(curl -I -H "Accept: application/vnd.docker.distribution.manifest.v2+json" \
  http://localhost:5000/v2/swarm-nginx/manifests/v1 | \
  grep Docker-Content-Digest | awk '{print $2}')

# Delete using digest
curl -X DELETE http://localhost:5000/v2/swarm-nginx/manifests/$DIGEST
```
