# Monitoring Stack — Docker Swarm (x86 / amd64 only)

> **amd64-only variant.** The swarm exporter (`ghcr.io/neuroforgede/docker-swarm-exporter`)
> publishes no arm64 image, so its service will not schedule on an arm64 node.
> For mixed-architecture or arm64 Swarm clusters use
> `dswarm/monitoring/grafana-vm-multiarch/` instead.
