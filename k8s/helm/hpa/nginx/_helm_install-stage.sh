...
# Configure for NodePort for testing JMeter
kubectl -n "${_n_space}" patch svc nginx-svc-"${_n_space}" -p '{"spec": {"type": "NodePort"}}'
minikube -n "${_n_space}" service nginx-svc-"${_n_space}" --url
